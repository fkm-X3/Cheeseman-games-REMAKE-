...
export async function createUser({ username, email, passwordHash }) {
  ...
  return firestore.runTransaction(async (tx) => {
    ...
    const emailLookupRef = firestore.collection(USER_LOOKUPS_COLLECTION).doc(`email:${emailLower}`);
...
export async function findUserByIdentifier(identifierLower) {
  const firestore = getDb();
  const lookupKey = identifierLower.includes("@")
    ? `email:${identifierLower}`
    : `username:${identifierLower}`;

  const lookupDoc = await firestore.collection(USER_LOOKUPS_COLLECTION).doc(lookupKey).get();
  if (!lookupDoc.exists) {
    return null;
  }
...
export async function submitScore({ userId, username, gameKey, score }) {
  const firestore = getDb();
...
export async function getLeaderboard({ gameKey, limit }) {
